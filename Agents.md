...
- **Static player export:** `POST /api/v3/export/:id` writes **data only** to `server/data/exports/{slug}/` (gitignored): `project.json` + `audio/*.mp3`. The player is served once from `server/export-template/` at `/player/` and loads any export via `?src=/exports/{slug}/project.json` (audio URLs resolve relative to the project file). Its NUI runtime maps to `/player/nui/` straight from the submodule — no vendored copies. Player ports the v3 playback core — virtual slide chunking, paragraph chaining, rAF highlighting, seekable `nui-slider` playhead, auto-advance. Fails loud (400 + gap list) unless every speakable paragraph has audio + alignment. Static hosting: deploy the template folder once (plus `nui/{nui.js, css/nui-theme.css, assets/material-icons-sprite.svg}`) + each export folder, link `player/?src=<path>/project.json`.
