...
- **Static player export:** `POST /api/v3/export/:id` writes a self-contained bundle to `server/data/exports/{slug}/` (gitignored): `index.html` + `player.js` + `player.css` (template: `server/export-template/`) + `project.json` + `audio/*.mp3`. The player is zero-dependency (no NUI runtime; theme values inlined) and ports the v3 playback core — virtual slide chunking, paragraph chaining, rAF highlighting, auto-advance. Fails loud (400 + gap list) unless every speakable paragraph has audio + alignment. Locally previewed at `/exports/{slug}/`; the folder runs on any static host.
