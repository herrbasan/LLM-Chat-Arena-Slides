...
- **Static player export:** `node pipeline/export.js <projectId> [outDir]` bakes a portable conversation folder (default `web-export/convos/{slug}/`, gitignored): `project.json` (messages, paragraphs, aligned word timings, voice mapping, source) + `transcript.md` (UTF-8 BOM) + `audio/*.mp3`. Fails loud unless every speakable paragraph has audio + alignment. The deployable unit is `web-export/` — sync the whole folder to any static host: `player/` (the player source, tracked; NUI runtime vendored under `nui/` — refresh by re-copying from the submodule), `convos/` (baked artifacts), `index.html` (generated conversation list, gitignored, refreshed on every export). Links are relative: `player/?src=../convos/{slug}/`. Locally the app serves the player at `/player/` and convos at `/exports/`. Player ports the v3 playback core — virtual slide chunking, paragraph chaining, rAF highlighting, seekable `nui-slider` playhead, auto-advance. This app is an internal tool; the products are recordings and these web exports.
