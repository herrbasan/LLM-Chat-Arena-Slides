...
- **Static player export:** `node pipeline/export.js <projectId> [outDir]` bakes a portable folder (default `server/data/exports/{slug}/`, gitignored): `project.json` (messages, paragraphs, aligned word timings, voice mapping, source) + `transcript.md` (UTF-8 BOM, human-readable with metadata header) + `audio/*.mp3`. Fails loud unless every speakable paragraph has audio + alignment. The player is `server/export-template/` — a self-contained static page (NUI runtime vendored under `nui/`; refresh by re-copying `nui.js`, `css/nui-theme.css`, `assets/material-icons-sprite.svg` from the submodule). Point it at a folder: `player/?src={path-to-folder}/`. Locally the app serves it at `/player/` (with `/player/nui/` mapped to the submodule) and export folders at `/exports/`. Player ports the v3 playback core — virtual slide chunking, paragraph chaining, rAF highlighting, seekable `nui-slider` playhead, auto-advance. This app is an internal tool; the products are recordings and these web exports.
